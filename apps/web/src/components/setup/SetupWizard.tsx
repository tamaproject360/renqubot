...
export function SetupWizard() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        setSaveState(`Draft tersimpan ${new Date().toLocaleTimeString('id-ID')}`);
        setDirty(false);
      } catch (error) {
        setSaveState(
  ...
  const goNext = async () => {
    ...
    try {
      ...
      setStepError(error instanceof Error ? error.message : 'Gagal menyimpan step.');
  ...
  const handleStartBot = async () => {
    ...
    try {
      ...
      setRuntimeState(`${status.message}${status.pid ? ` PID ${status.pid}` : ''}`);
    } catch (error) {
      setRuntimeState(
        error instanceof Error ? error.message : 'Gagal menyalakan bot runtime.',
      );
    }
  };
    ...
    <>
      ...
      <div className="wizard-shell">
        ...
        >
          ...
          <div className="wizard-panel">{renderStep(activeStep, form, updateField)}</div>
...
const renderStep = (
  ...
  if (step === 0) {
    return (
      <div className="form-grid">
        <FormField label="Provider AI Aktif" hint="Contoh: Gemini untuk setup awal karena runtime bot saat ini sudah stabil dengan Gemini.">
          <select
            className="select"
            value={form.activeAiProvider}
            onChange={(event) => updateField('activeAiProvider', event.target.value)}
          >
        ...
        <FormField label="Database URL" hint="Contoh: file:./data/baileys.db untuk SQLite lokal.">
          <input
            className="input"
        ...
        <FormField label="Allowed User IDs" hint="Contoh: 6281234567890,6289876543210. Kosongkan jika semua pengirim boleh memakai bot.">
          <input
            className="input"
            placeholder="6281234567890,6289876543210"
            value={form.allowedUserIds}
            onChange={(event) => updateField('allowedUserIds', event.target.value)}
          />
        </FormField>
      </div>
    );
  ...
  if (step === 1) {
    return (
      <div className="form-grid">
        ...
        <FormField label={`${providerLabels[form.activeAiProvider]} API Key`} hint="Contoh Gemini: AIza... Jangan commit API key ke Git; secret akan disimpan lewat endpoint khusus.">
          <input
            className="input"
  ...
  if (step === 2) {
    return (
      <div className="form-grid">
        <FormField label="Spreadsheet ID" hint="Contoh: ambil dari URL Google Sheets, bagian setelah /d/ dan sebelum /edit.">
          <input
            className="input"
        ...
        <FormField label="Spreadsheet Name" hint="Contoh: infos. Ini nama/konteks file spreadsheet untuk konfigurasi aplikasi.">
          <input
            className="input"
            placeholder="infos"
            value={form.spreadsheetName}
            onChange={(event) => updateField('spreadsheetName', event.target.value)}
          />
        </FormField>
        <FormField label="Sheet Tab Name" hint="Contoh: Logs. Bot akan membuat header transaksi di tab ini.">
          <input
            className="input"
            placeholder="Logs"
        ...
        <FormField label="Service Account File Name" hint="Contoh: google-service-account.json. File akan disimpan di data/credentials.">
          <input
            className="input"
            placeholder="google-service-account.json"
            value={form.serviceAccountFileName}
            onChange={(event) => updateField('serviceAccountFileName', event.target.value)}
          />
        </FormField>
        <FormField label="Service Account JSON" hint='Contoh: { "type": "service_account", "project_id": "renqu-bot" }. Paste isi JSON credential dari Google Cloud.'>
          <textarea
            className="textarea"
            placeholder='{ "type": "service_account", "project_id": "renqu-bot", "private_key": "..." }'
            value={form.serviceAccountContent}
            onChange={(event) => updateField('serviceAccountContent', event.target.value)}
          />
        </FormField>
      </div>
    );
  ...
  if (step === 3) {
    return (
      <div className="wizard-callout">
        ...
        <p>
          Setelah bot runtime dinyalakan di step Review, buka halaman WhatsApp untuk melihat QR dan scan dengan akun WhatsApp yang akan dipakai bot.
        </p>
        <p className="card__meta">
          Contoh alur: klik Simpan & Nyalakan Bot, buka menu WhatsApp, scan QR, lalu kirim pesan seperti "Beli gorengan 10000" dari nomor yang diizinkan.
        </p>
      </div>
    );
  }
...
const renderProviderFields = (
    ...
    return (
      <>
        <FormField label="OpenAI Model" hint="Contoh: gpt-4.1-mini.">
          <input className="input" value={form.openaiModel} onChange={(event) => updateField('openaiModel', event.target.value)} />
        </FormField>
        <FormField label="OpenAI Base URL" hint="Opsional. Contoh custom gateway: https://api.openai.com/v1.">
          <input className="input" placeholder="https://api.openai.com/v1" value={form.openaiBaseUrl} onChange={(event) => updateField('openaiBaseUrl', event.target.value)} />
        </FormField>
      </>
    );
  }
  ...
  if (form.activeAiProvider === 'anthropic') {
    return (
      <>
        <FormField label="Anthropic Model" hint="Contoh: claude-3-5-haiku-latest.">
          <input className="input" value={form.anthropicModel} onChange={(event) => updateField('anthropicModel', event.target.value)} />
        </FormField>
        <FormField label="Anthropic Base URL" hint="Opsional. Kosongkan untuk endpoint default.">
          <input className="input" placeholder="https://api.anthropic.com" value={form.anthropicBaseUrl} onChange={(event) => updateField('anthropicBaseUrl', event.target.value)} />
        </FormField>
      </>
    );
  }
  ...
  if (form.activeAiProvider === 'openai-compatible') {
    return (
      <>
        <FormField label="Custom Provider Name" hint="Contoh: OpenRouter, Groq, atau LiteLLM Gateway.">
          <input className="input" value={form.customProviderName} onChange={(event) => updateField('customProviderName', event.target.value)} />
        </FormField>
        <FormField label="Custom Model" hint="Contoh: openai/gpt-4.1-mini atau llama-3.1-70b-versatile.">
          <input className="input" placeholder="openai/gpt-4.1-mini" value={form.customModel} onChange={(event) => updateField('customModel', event.target.value)} />
        </FormField>
        <FormField label="Custom Base URL" hint="Wajib untuk provider compatible. Contoh: https://openrouter.ai/api/v1.">
          <input className="input" placeholder="https://openrouter.ai/api/v1" value={form.customBaseUrl} onChange={(event) => updateField('customBaseUrl', event.target.value)} />
        </FormField>
      </>
    );
  }
  ...
  return (
    <>
      <FormField label="Gemini Model" hint="Contoh: gemini-2.0-flash-lite untuk biaya ringan, atau gemini-2.5-flash untuk kualitas lebih tinggi.">
        <input className="input" value={form.geminiModel} onChange={(event) => updateField('geminiModel', event.target.value)} />
      </FormField>
      <FormField label="Gemini Base URL" hint="Opsional. Kosongkan untuk endpoint default Google Gemini, atau isi URL gateway internal.">
        <input className="input" placeholder="https://generativelanguage.googleapis.com" value={form.geminiBaseUrl} onChange={(event) => updateField('geminiBaseUrl', event.target.value)} />
      </FormField>
    </>
  );
};
...
function ReviewStep({ form }: { form: ISetupFormState }) {
  return (
    <div className="review-grid">
      <ReviewItem label="Provider AI" value={providerLabels[form.activeAiProvider]} />
      <ReviewItem label="Database URL" value={form.databaseUrl} />
      <ReviewItem label="Allowed Users" value={form.allowedUserIds || 'Semua user diizinkan'} />
      <ReviewItem label="Model Aktif" value={getActiveModel(form)} />
      <ReviewItem label="Spreadsheet ID" value={form.spreadsheetId || 'Belum diisi'} />
      <ReviewItem label="Sheet Tab" value={form.sheetName} />
      <ReviewItem label="Service Account" value={form.serviceAccountContent ? form.serviceAccountFileName : 'Belum diupload'} />
      <ReviewItem label="Bot Runtime" value="Klik tombol Simpan & Nyalakan Bot untuk menjalankan runtime." />
      {form.activeAiProvider !== 'gemini' ? (
        <p className="form-error">
          Catatan: tombol start runtime saat ini hanya mendukung provider Gemini. Provider lain sudah bisa disimpan sebagai konfigurasi, tetapi runtime legacy belum menjalankannya.
        </p>
      ) : null}
    </div>
  );
...
const persistCurrentStep = async (step: number, form: ISetupFormState) => {
  ...
  if (step === 2 && form.serviceAccountContent.trim()) {
    await uploadGoogleServiceAccount({
      fileName: form.serviceAccountFileName.trim() || 'google-service-account.json',
...
const validateStep = (step: number, form: ISetupFormState) => {
  if (step === 0) {
    ...
    if (!splitAllowedUsers(form.allowedUserIds).every((item) => /^\d{8,15}$/.test(item))) {
  ...
  if (step === 1) {
    ...
    if (form.activeAiProvider === 'openai-compatible' && !form.customBaseUrl.trim()) {
